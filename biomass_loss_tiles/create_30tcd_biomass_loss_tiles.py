...
def mask_loss(tile_id):
    ...
    biomass_tile = 's3://gfw-files/sam/carbon_budget/carbon_111717/carbon/{}_carbon.tif'.format(tile_id)
    cmd = ['aws', 's3', 'cp', biomass_tile, '.' ]
    ...
    cmd = ['gdal_calc.py', '-A', loss_tile, '-B', tcd_tile, '-C', biomass_tile, calc, outfile, '--NoDataValue=0', '--co', 'COMPRESS=LZW']
